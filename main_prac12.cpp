...
void animacion()
{
	...
	if(circuito)//COMIENZA 
	{
		if(recorrido1)//variable de tipo booleano para indicar en que estado nos ecnontramos ESTADO A
		{
			if (!rev)
			{
				    rotKit = 0;
				    movKitZ++;
				if (movKitZ > 155) //transicion a estado B
				{
					recorrido1 = false;
					recorrido2 = true;
				}
		...
		if(recorrido2)//ESTADO B
		{
			if (!rev)
			{
				...
				rotKit = 90;//numero albitrario que dice cuanto gira
				movKitX++;
		...
		if (recorrido5)//ESTADO E
		{
			if (!rev)
			{
				...
				if (movKitZ > 0)//transicion en estado A
				{
					recorrido5 = false;
					recorrido1 = true;
				}
...
//r cambiara la direccion el vehivulo, para ir a sentido contrario
//ejercicio de casa: al mismo circuito modificarla, cuando llegue a D sera por arriba (volando)
//llantas h
