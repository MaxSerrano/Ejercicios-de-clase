...
/*
manual
B= pierna izquierda atras
b= pierna izquierda adelante
V= pierna derecha atras
v= piernda derecha adelante
N=brazo derecho atras
n=brazo derecho adelante
M=brazo izquierdo atras
m=brazo izquierdo adelante
C = cabeza derecha
c = cabeza izquierda
X = cadera a la derecha
x= cadera a la izquierda
*/
//******22,42,170,186,197,312,1446,1558,1562****************************//
...
float posX =0, posY = 2.5, posZ =-3.5, rotRodIzq = 0, rotRodDer = 0, movBIzq = 0, movCIzq = 0, movBDer = 0, movCDer = 0, movC = 0, movCad = 0; 
float giroMonito = 0;
...
typedef struct _frame  //CREACION DE UNA ESTRUCTURA 
{
	//Variables para GUARDAR Key Frames
	float posX;		//Variable para PosicionX
	float posY;		//Variable para PosicionY
	float posZ;		//Variable para PosicionZ
	float incX;		//Variable para IncrementoX AYUDA AL CALCULO DE INTERPOLACION
	float incY;		//Variable para IncrementoY  POR CADA VARIABLE SU RESPECTIVO INCREMENTO
	float incZ;		//Variable para IncrementoZ
	float rotRodIzq;
	float rotInc;
	float giroMonito;
	float giroMonitoInc;
	float rotRodDer; //se crea la misma variable
	float rotRodDerInc;
	//brazo izquiero
	float movBIzq; 
	float movBIzqInc;
	...
	//brazo derecho
	float movBDer; //se crea la misma variable
	float movBDerInc;
	//codoDer
	float movCDer;
	float movCDerInc;
...
FRAME KeyFrame[MAX_FRAMES]; //TAMAÑO QUE ES 
int FrameIndex=0;			//introducir datos
bool play=false; //variable para ejecutar animacion
int playIndex=0; //para que produzca animación


//NEW//////////////////NEW//////////////////NEW//////////////////NEW////////////////NEEEEEEEEEEEEEEEEEEEEEEEEEEEEEWW
...
void saveFrame ( void ) //guardar frame 
{
	
	printf("frameindex %d\n",FrameIndex);			

	KeyFrame[FrameIndex].posX=posX; //todas la variables se manda la estructura
	KeyFrame[FrameIndex].posY=posY;//todas las variables
	KeyFrame[FrameIndex].posZ=posZ;

	KeyFrame[FrameIndex].rotRodIzq=rotRodIzq; //regreo a posicion de el monito
	KeyFrame[FrameIndex].giroMonito=giroMonito;

	KeyFrame[FrameIndex].rotRodDer=rotRodDer; //
	//brazoDer
	KeyFrame[FrameIndex].movBDer = movBDer; //
	KeyFrame[FrameIndex].movCDer = movCDer; //
	//brazoIzq
	KeyFrame[FrameIndex].movBIzq = movBIzq ; //
	KeyFrame[FrameIndex].movCIzq = movCIzq; //
	//cabeza
	KeyFrame[FrameIndex].movC =movC; //
	//cadera
	KeyFrame[FrameIndex].movCad = movCad; //

			
	FrameIndex++; //incrementa valor de frame
}
...
void resetElements( void )
{
	...
	//brazo derecho
	movBDer = KeyFrame[0].movBDer;
	movCDer = KeyFrame[0].movCDer;
	//brazo izquierdo
	movBIzq = KeyFrame[0].movBIzq;
	movCIzq = KeyFrame[0].movCIzq;
	//cabeza
	movC = KeyFrame[0].movC;
	//cadera
	movCad = KeyFrame[0].movCad;
}

void interpolation ( void ) //intermedio en cada key frame
{
	KeyFrame[playIndex].incX = (KeyFrame[playIndex + 1].posX - KeyFrame[playIndex].posX) / i_max_steps;	 //velocidad de animacón , resta de dos posiciones
	KeyFrame[playIndex].incY = (KeyFrame[playIndex + 1].posY - KeyFrame[playIndex].posY) / i_max_steps;	//y se guarda en variable de incremento
	...
	//brazoder
	KeyFrame[playIndex].movBDerInc = (KeyFrame[playIndex + 1].movBDer - KeyFrame[playIndex].movBDer) / i_max_steps;
	KeyFrame[playIndex].movCDerInc = (KeyFrame[playIndex + 1].movCDer - KeyFrame[playIndex].movCDer) / i_max_steps;
	//brazoIzq
	KeyFrame[playIndex].movBIzqInc = (KeyFrame[playIndex + 1].movBIzq - KeyFrame[playIndex].movBIzq) / i_max_steps;
	KeyFrame[playIndex].movCIzqInc = (KeyFrame[playIndex + 1].movCIzq - KeyFrame[playIndex].movCIzq) / i_max_steps;
	//cabeza
	KeyFrame[playIndex].movCInc = (KeyFrame[playIndex + 1].movC - KeyFrame[playIndex].movC) / i_max_steps;
	//cadera
	KeyFrame[playIndex].movCadInc = (KeyFrame[playIndex + 1].movCad - KeyFrame[playIndex].movCad) / i_max_steps;
...
void monito() //variables que se  esta utilizando
{
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix();//Cintura
			...
			glPushMatrix(); //Pie Derecho -->
				...
				glPushMatrix();
					glTranslatef(0, -0.5, 0);
					glRotatef(15+rotRodDer, 1, 0, 0); //new
...
void animacion()
{
	...
	{		
		...
		{
			//Draw animation por cada frame se hace su interpolacion 
			...
			rotRodDer += KeyFrame[playIndex].rotRodDerInc; //new

			movBDer += KeyFrame[playIndex].movBDerInc;
			movBIzq += KeyFrame[playIndex].movBIzqInc;
			movC += KeyFrame[playIndex].movCInc;
			movCad += KeyFrame[playIndex].movCadInc;

			i_curr_steps++; //se pasan todos los valores 
		}
...
/* monito por animacion key frame
 tecla B mueve la pierna
 ATRAS Y ADELANTE
 TECLA L REPRODUCE LA ANIMACIÓN
 k ES DE KEY FRAME HACE QUE "GRABE LOS MOVIMIENTOS POR SEGMENTOS

 poner al menos 30 keyframes
*/